...
export default class RegistrationForm {
    ...
    usernameImmediately(){
       ...
       if(this.username.value.length > 25){
            this.showValidationError(this.username, "Username can not exceed 25")
       }
    ...
    emailAfterDelay(){
        ...
        if(!this.email.errors){
            axios.post('/doesEmailExist', { _csrf: this._csrf, email: this.email.value}).then((response) => {
                if(response.data){
                    this.email.isUnique = false
                    this.showValidationError(this.email, "This email id is already taken.")
                }else{
    ...
    usernameAfterDelay(){
        if(this.username.value.length < 3){
            this.showValidationError(this.username, "Username should be atleast 3 characters long !")
        }
    ...
    passwordImmediately(){
        if (this.password.value.length > 50){
            this.showValidationError(this.password, "Password cannot excceed 50 characters")
        }
